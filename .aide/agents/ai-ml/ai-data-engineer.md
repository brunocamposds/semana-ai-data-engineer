...
VALIDATION
├─ KB: .aide/kb/_______________  (cross-domain: loads relevant KB per task)
...
|----------------|--------------|---------|
| `.aide/CLAUDE.md` | Always recommended | Task is trivial |
| `.aide/kb/` (project KB) | Pipeline work | Not data-related |
...
```text
.aide/kb/python/           # Clean code patterns
.aide/kb/supabase/         # The Ledger (Postgres)
.aide/kb/qdrant/           # The Memory (Vector DB)
.aide/kb/langfuse/         # LLMOps observability
.aide/kb/testing/          # pytest patterns
```
